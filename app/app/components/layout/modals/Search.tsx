...
export default function Search() {
  ...
  return (
    <div className="flex items-center gap-4 relative">
      ...
      <button
        type="button"
        className="h-[3rem] w-[18rem] bg-[#2f3549] text-sm text-white p-2 rounded-md flex justify-between items-center"
      ...
      {isDropdownOpen && (
        <ul className="w-[18rem] bg-[#2a3042] p-2 rounded-md flex flex-col gap-2 absolute top-[4rem] right-0 transition-all duration-150 z-10">
          {mapOptions.map((option, index) => (
            <li
              key={index}
              className="hover:bg-[#2f3549] p-2 rounded-md cursor-pointer flex justify-between items-center transition-all duration-150"
